...
//--------------------------------------------------------------
LayerDescription::LayerDescription(unsigned int id, const std::filesystem::path& path, midiNote note, midiNote cc, int alpha, const BlendMode& blendMode)
  : MappableDescription(note == invalid_midi ? id + LAYER_NOTE_OFFSET : note,
...
EffectDescription::EffectDescription(int id, EffectType type, midiNote note, midiNote cc)
    : MappableDescription(note == invalid_midi ? static_cast<int>(type) + EFFECT_NOTE_OFFSET : note,
