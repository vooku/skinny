...
static const auto computeShader =
    ...
    "	vec3 colors[7];\n"
    "	colors[0] = alphas[0] * imageLoad(layer0, ivec2(xy / globalDims * dimensions[0])).rgb;\n"
    "	colors[1] = alphas[1] * imageLoad(layer1, ivec2(xy / globalDims * dimensions[1])).rgb;\n"
    "	colors[2] = alphas[2] * imageLoad(layer2, ivec2(xy / globalDims * dimensions[2])).rgb;\n"
    "	colors[3] = alphas[3] * imageLoad(layer3, ivec2(xy / globalDims * dimensions[3])).rgb;\n"
    "	colors[4] = alphas[4] * imageLoad(layer4, ivec2(xy / globalDims * dimensions[4])).rgb;\n"
    "	colors[5] = alphas[5] * imageLoad(layer5, ivec2(xy / globalDims * dimensions[5])).rgb;\n"
    "	colors[6] = alphas[6] * imageLoad(layer6, ivec2(xy / globalDims * dimensions[6])).rgb;\n"
    ...
    "					break;\n"
    "			}\n"
    "			blended = clamp(vec3(0.0), masterAlpha * vec3(1.0), blended);\n"
    "		}\n"
    "	}\n"
    "	if (anyPlaying) {\n"
