...
static const auto computeShader =
    ...
    "				case 1: {// Multiply\n"
    "					vec3 tmp = blended * colors[i];\n"
    "                   blended = mix(blended, tmp, alphas[i]);\n"
    "					break;}\n"
    "				case 2: {// Screen\n"
    "					vec3 tmp = 1.0 - (1.0 - blended) * (1.0 - colors[i]);\n"
    "                   blended = mix(blended, tmp, alphas[i]);\n"
    "					break;}\n"
    "				case 3: {// Darken\n"
    "					vec3 tmp = min(blended, colors[i]);\n"
    "                   blended = mix(blended, tmp, alphas[i]);\n"
    "					break;}\n"
    "				case 4: {// Lighten\n"
    "					vec3 tmp = max(blended, colors[i]);\n"
    "                   blended = mix(blended, tmp, alphas[i]);\n"
    "					break;}\n"
    "				case 5: {// LinearDodge\n"
    "					vec3 tmp = blended + colors[i];\n"
    ...
    "					break;}\n"
    "				case 6: {// Difference\n"
    "					vec3 tmp = abs(blended - colors[i]);\n"
    "                   blended = mix(blended, tmp, alphas[i]);\n"
    "					break;}\n"
    "				case 7: {// Exclusion\n"
    "					vec3 tmp = blended + colors[i] + 2.0 * blended * colors[i];\n"
    "                   blended = mix(blended, tmp, alphas[i]);\n"
    "					break;}\n"
    "				default:{\n"
    "					vec3 tmp = colors[i];\n"
    ...
    "					break;}\n"
    "			}\n"
    "			blended = masterAlpha * clamp(vec3(0.0), vec3(1.0), blended);\n"
    "		}\n"
    "	}\n"
    "	if (anyPlaying) {\n"
